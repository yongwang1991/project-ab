    ...
    if prompt := st.chat_input("What is up?"):
        if st.session_state.stage["current"] <= st.session_state.stage["max"]:
            ...
            ##! check if user is agreeable with llm suggestions for the main headers !##
                ...
                decision_json = llm.get_completion_by_messages(prompt)
                ...
                if proceed_decision == True:
                    ...
                    response_json = llm.get_completion_by_messages(system_prompts.system_prompts["proceed"])
                    response = json.loads(response_json)
            ...
            ##! confirmation of drafting of each section !##
                ...
                #* if user is NOT agreeable with the main headers *#
                    ...
                    system_instructions = [{"role": "system", "content": f"You are currently amending the requirements specifications for the {current_section} section. The current draft in demarcated by the tag <draft></draft> below. <draft>{st.session_state.current_draft}</draft> " + system_prompts.system_prompts["initial_draft"]}]
                    built_prompt = st.session_state.background + st.session_state.messages + system_instructions
                    response_json = llm.get_completion_by_messages(built_prompt)
                    response = json.loads(response_json)
